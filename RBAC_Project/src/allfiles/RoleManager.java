...
public class RoleManager implements Repository<Role> {
    ...
    public void addPermissionToRole(String roleName, Permission permission) {
        synchronized (object) {
            ...
            if (role == null) {
                throw new IllegalArgumentException("java.Role with name '" + roleName + "' not found!");
            }
    ...
    public void add(Role role) {
        if (role == null) throw new IllegalArgumentException("java.Role cannot be null");
        if (role.name() == null || role.name().trim().isEmpty()) {
            throw new IllegalArgumentException("java.Role name cannot be empty");
        }
        if (role.id == null || role.id.trim().isEmpty()) {
            throw new IllegalArgumentException("java.Role id cannot be empty");
        }
        synchronized (object) {
            if (rolesById.containsKey(role.id)) {
                throw new IllegalArgumentException("java.Role with id '" + role.id + "' already exists!");
            }
            if (rolesByName.containsKey(role.name())) {
                throw new IllegalArgumentException("java.Role with name '" + role.name() + "' already exists!");
            }
